...
class WordInfo:
    ...
    def get_meaning(self, word: str) -> Dict[str, List[Dict[str, Any]]]:
        """
        Returns meanings and part-of-speech for a given word
        """
        try:
        ...
        if not sections:
            return {
                soup.find("span", {"class": "luna-pos"})
                .text.strip()
                .upper()
                .replace(",", ""): self._meanings(soup)
            }
        return {
            s.find("span", {"class": "luna-pos"})
            .text.strip()
            .upper()
            .replace(",", ""): self._meanings(s)
            for s in sections
        }

    def get_synonym(self, word: str) -> Union[None, List[str]]:
        """
    ...
    def get_antonym(self, word: str) -> Union[None, List[str]]:
        """
    ...
    def get_part_of_speech(self, word: str) -> Union[List[str], str]:
        ...
        try:
            ...
            return self._open_source_get_part_of_speech(word)
        ...
        return [
            s.find("span", {"class": "luna-pos"}).text.strip().upper().replace(",", "")
            for s in sections
        ]

    def _split_words_and_examples(
        self, sentencelist: List[str]
    ) -> List[Dict[str, Any]]:
        """
        ...
        for word in sentencelist:
            if len(word.split(":")) > 1:
                result.append(
                    {"meaning": word.split(":")[0], "examples": word.split(":")[1]}
                )
            else:
                if "!" in word:
                    pass
    ...
    def _meanings(self, section: ResultSet) -> List[Dict[str, Any]]:
        ...
        if not default_content:
            ...
            list_of_definitions_and_examples_span = [
                s.text.strip().replace(".", "")
                for s in spans
                if "Slang" not in s.text.strip().replace(".", "")
            ][0:3]
        ...
        list_of_definitions_and_examples_default_content = [
            "".join(
                [
                    word
                    for word in d.text.strip().split(".")
                    if "(def" not in word and "1)" not in word
                ]
            )
            for d in definitions
        ][0:3]

        return self._split_words_and_examples(
            list_of_definitions_and_examples_default_content
        )

    def _get_open_source_dataset(self, word: str) -> Union[Dict, None]:
        """
        ...
        abs_path = os.path.join(
            "wordset_open_source_data", f"{first_letter_of_word}.json"
        )
    ...
    def _open_source_get_meaning(self, word: str) -> Dict[str, List[Dict[str, Any]]]:
        ...
        for d in original_meanings_data:
            ...
            d["examples"] = None
        result = {
            k.upper(): [d for d in original_meanings_data if d.get("speech_part") == k]
            for k in part_of_speech_list
        }
        return {
            k: [{"meaning": d.get("meaning"), "examples": d.get("examples")} for d in v]
    ...
    def _open_source_get_part_of_speech(self, word: str) -> List[str]:
        ...
        result = []
        [
            result.append(d["speech_part"].upper())
            for d in original_meanings_data
            if d["speech_part"].upper() not in result
        ]
        return result
